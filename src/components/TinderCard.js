...
const limited_finalX = (finalX) => {
  const isNegative = finalX < 0
  const MAX_VALUE = 4000
  if (Math.abs(finalX) > MAX_VALUE) {
    return isNegative ? -MAX_VALUE : MAX_VALUE
  } else {
    return finalX
  }
}
...
const animateOut = async (gesture, setSpringTarget, windowHeight, windowWidth) => {
  ...
  const velocity = DEFAULT_VELOCITY || pythagoras(gesture.x, gesture.y)
  const finalX = limited_finalX(diagonal * 2.5 * gesture.x)
  const finalY = diagonal * 2.5 * gesture.y
  const finalRotation = gesture.x * 45
  const duration = diagonal / (velocity * 5)
...
const getSwipeDirection = (property) => {
  // console.log('property.x > settings.swipeThreshold', property.x > settings.swipeThreshold)
  // console.log('property.x, settings.swipeThreshold', property.x, settings.swipeThreshold, property.y)

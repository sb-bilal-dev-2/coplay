...
const HorizontalScrollMenu = ({ items, baseRoute, verticalCard }) => {
    ...
    const handleScrolled = createDebouncedFunction(() => {
        console.log(scrollRef?.current?.offsetWidth, scrollRef?.current?.scrollLeft, scrollRef?.current?.scrollWidth)
        const newHasScrolledEnd = Math.ceil(scrollRef?.current?.offsetWidth + scrollRef?.current?.scrollLeft) === scrollRef?.current?.scrollWidth;
    ...
    return (
        <div className="horizontal-scroll-menu">
            {!hasScrolledStart &&
                <button
                    style={{ background: "linear-gradient(-90deg, rgba(0, 0, 0, 0.1), rgba(0, 0, 0, 0.7))" }}

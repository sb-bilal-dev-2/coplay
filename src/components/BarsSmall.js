...
export const BarsSmall = ({ barsColor }) => {
  return (
    <button className="BarsSmall w-8 h-8 relative cursor-pointer">
      ...
      <div className="absolute inset-0 flex items-center justify-center">
        <div className="w-8 h-8 rounded-full flex flex-col items-center justify-center">
          <div className="w-5 h-0.5 bg-white mb-2 rounded" style={{ background: barsColor }}></div>
          <div className="w-5 h-0.5 bg-white rounded" style={{ background: barsColor }}></div>
        </div>
      </div>
    </button>
  );
